document.addEventListener("DOMContentLoaded", () => {
    ...
    function M() {
        ...
        filteredTools.slice((P - 1) * A, P * A).forEach(tool => {
            ...
            card.innerHTML = `<img src="${tool.logo}" alt="${tool.name} Icon" class="tool-thumbnail"><span class="tool-name">${tool.name}</span><span class="tool-category">${tool.category}</span><span class="tool-date">${tool.date_added}</span><div class="rating">${"★".repeat(Math.floor(tool.popularity)).padEnd(5, "☆")}</div><a href="${tool.url}" target="_blank" class="tool-link">🌐</a><span class="tool-check ${isBookmarked ? "tool-bookmark" : ""}" data-id="${tool.id}">${isBookmarked ? "⭐" : "✅"}</span><span class="quick-favorite" style="color: ${isFavorite ? '#ffca28' : '#ccc'}">⭐</span>`;
        ...
        bookmarks.forEach(tool => {
            const card = document.createElement("div");
            card.classList.add("tool-card");
            card.innerHTML = `<img src="${tool.logo}" alt="${tool.name} Icon" class="tool-thumbnail"><span class="tool-name">${tool.name}</span><span class="tool-category">${tool.category}</span><span class="tool-date">${tool.date_added}</span><div class="rating">${"★".repeat(Math.floor(tool.popularity)).padEnd(5, "☆")}</div><a href="${tool.url}" target="_blank" class="tool-link">🌐</a><span class="tool-check tool-bookmark">⭐</span>`;
            tool.tags.forEach(tag => {
                const tagSpan = document.createElement("span");
                tagSpan.classList.add("tool-tag");
                tagSpan.textContent = tag;
    ...
    x.addEventListener("click", () => {
        ...
        freeTools.forEach(tool => {
            const card = document.createElement("div");
            card.classList.add("tool-card");
            card.innerHTML = `<img src="${tool.logo}" alt="${tool.name} Icon" class="tool-thumbnail"><span class="tool-name">${tool.name}</span><span class="tool-category">${tool.category}</span><span class="tool-date">${tool.date_added}</span><div class="rating">${"★".repeat(Math.floor(tool.popularity)).padEnd(5, "☆")}</div><a href="${tool.url}" target="_blank" class="tool-link">🌐</a>`;
            tool.tags.forEach(tag => {
                const tagSpan = document.createElement("span");
                tagSpan.classList.add("tool-tag");
                tagSpan.textContent = tag;
