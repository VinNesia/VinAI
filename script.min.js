document.addEventListener("DOMContentLoaded", () => {
    ...
    function M() {
        ...
        o.slice((P - 1) * A, P * A).forEach(e => {
            ...
            t.innerHTML = `<img src="${e.logo}" alt="${e.name} Icon" class="tool-thumbnail"><span class="tool-name">${e.name}</span><span class="tool-category">${e.category}</span><span class="tool-date">${e.date_added}</span><div class="rating">${"★".repeat(Math.floor(e.popularity)).padEnd(5, "☆")}</div><a href="${e.url}" target="_blank" class="tool-link">🌐</a><span class="tool-check ${o ? "tool-bookmark" : ""}" data-id="${e.id}">${o ? "⭐" : "✅"}</span><span class="quick-favorite" style="color: ${isFavorite ? '#ffca28' : '#ccc'}">⭐</span>`,
                e.tags.forEach(e => {
    ...
    document.querySelector('.menu').addEventListener('click', () => {
        ...
        menu.innerHTML = '<a href="index.html">Home</a><a href="about.html">About</a><a href="blog.html">Blog</a><a href="contact.html">Contact</a><a href="faq.html">FAQ</a><a href="tutorial.html">Tutorial</a><a href="forum.html">Forum</a><a href="profile.html">Profil</a>';
