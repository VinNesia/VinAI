document.addEventListener("DOMContentLoaded", () => {
    ...
    // Fungsi animasi counter dengan mode awal dan random
    function animateCounter(target, element, initialDuration, interval) {
        let current = 0;
        let isInitial = true;
        const range = target - current;
        const increment = target / (initialDuration / 16); // 16ms per frame (60fps)
        ...
        function update() {
            ...
            if (isInitial) {
                ...
                if (progress >= 1) {
                    current = target;
                    isInitial = false;
                    setTimeout(startRandomCounter, interval * 1000); // Mulai random setelah interval
                }
            } else {
                current += Math.floor(Math.random() * 10) + 1; // Naik random 1-10
            }
    ...
    if (statsText) {
        animateCounter(38345, statsText, 2000, 2); // Target 38,345, durasi awal 2 detik, interval random 2 detik
    }
    ...
    fetch("tools.json").then(e => {
        ...
        t.forEach(e => {
            ...
            a.addEventListener("change", t => {
                ...
                // Restart animasi saat bahasa berubah
                if (statsText) {
